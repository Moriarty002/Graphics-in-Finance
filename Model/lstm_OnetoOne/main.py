...
def readData(df,column='收盤價', n=30, all_too=True, index=False, train_end=-300):
...
def OneToOne(stockcode):
    n = 30
    LR = 0.0001
    EPOCH = 100
    train_end = -500
    # 数据集建立
    df1=Tools.checkCodeInDir(stockcode)
    df, df_all, df_index = readData(df1,'收盤價', n=n, train_end=train_end)

    df_all = np.array(df_all.tolist())
    plt.plot(df_index, df_all, label='raw-data')
    ...
    for step in range(EPOCH):
        for tx, ty in trainloader:
            ...
            #將cuda拿掉可改用cpu
            x = torch.unsqueeze(tx, dim=0).cuda()
            y = ty.cuda()
